...
namespace ToolBox.Services.LicenseManagerProd
{
    public class MFilesUsersService
    {
        ...
        public bool areValidCredentials(string username, string password)
        {
            ...
            GroupPrincipal group = GroupPrincipal.FindByIdentity(pc, "GRP Service des Systèmes d'Information");
            UserPrincipal user = UserPrincipal.FindByIdentity(pc, username);

            // Traitement
            if (!pc.ValidateCredentials(username, password) || !group.GetMembers(true).Contains(user))
            {
