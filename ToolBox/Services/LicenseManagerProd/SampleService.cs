        ...
        public async Task DoSomethingAsync()
        {
            ...
            if (rightHour())
            {
                ...
                if (cs.getConf().active)
                {
                    ...
                    Console.WriteLine($"Deleting the licenses for LicenseManagerProd at {DateTime.Now.ToString("HH:mm")}");
                }
                else
                {
                    Console.WriteLine($"Updating the list for LicenseManagerProd at {DateTime.Now.ToString("HH:mm")}");
                    la.updateList(mFilesUsersService.getList(cs.getConf().groups));
                }
